...
void dalijimas(std::list<data>& mok, int paz, std::list<data>& vargsiukai, std::list<data>& moksliukai) {
    ...
    //std::cout << mok.size();
    int d = mok.size();
    std::list<data>::iterator it = mok.begin();
    /*mok.sort([](const data& d1, const data& d2) {
        return d1.vardas < d2.vardas;
        });*/
    for (int i = 0; i < d; i++) {
        it->rezult = (it->rezult / it->n * 0.4) + (0.6 * it->egz);
        if (it->rezult < 5) {
...
{   
    ...
    mokslai << "----------------------------------------------------------------------------------------" << std::endl;
    int i = 0;
    auto iv = vargsiukai.begin();
    for (data& m : vargsiukai)
    {   
        rikiavimas(vargsiukai, i, paz,iv);
    ...
    i = 0;
    auto im = moksliukai.begin();
    for (data& m : moksliukai)
    {   
       rikiavimas(moksliukai, i, paz,im);
...
void rikiavimas(std::list<data>& mok, int a, int paz,auto it)
{   
    int laikinas;
